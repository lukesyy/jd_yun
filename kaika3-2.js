...
!(async () => {
	...
	authorCodeList = ['4051a254560e45018fe2e059489e183d']
...
async function takePostRequest(type) {
	...
	switch (type) {
		...
		case 'accessLogWithAD':
			...
			body = `venderId=${$.shopId || $.venderId || ''}&code=99&pin=${encodeURIComponent($.Pin)}&activityId=${
				$.activityId
		...
		case 'activityContent':
			...
			body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&pinImg=${encodeURIComponent($.attrTouXiang)}&nick=${encodeURIComponent(
				$.nickname
		...
		case 'addSku':
			...
			body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${
				$.actorUuid
				...
				if (typeof res == 'object') {
					if (res.result && res.result === true) {
						if (res.data && typeof res.data.yunMidImageUrl != 'undefined')
							$.attrTouXiang =
								res.data.yunMidImageUrl || 'https://img10.360buyimg.com/imgzone/jfs/t1/7020/27/13511/6142/5c5138d8E4df2e764/5a1216a3a5043c5d.png'
					} else if (res.errorMessage) {
						console.log(`${type} ${res.errorMessage || ''}`)
					} else {
						console.log(`${type} ${data}`)
					...
					if (res.result && res.result === true) {
						if (typeof res.data == 'object') {
							...
							if (type == 'followShop') {
								...
								msg =
									(res.data.drawOk == true && ((res.data.drawInfoType == 6 && res.data.name) || '')) ||
									(res.data.wdsrvo.drawOk == true && ((res.data.wdsrvo.drawInfoType == 6 && res.data.wdsrvo.name) || '')) ||
									'空气💨'
							}
							if (!msg) {
								msg = '空气💨'
					...
					if (res.result && res.result === true) {
						...
						for (let i in res.data) {
							...
							if (item.drawId == 'dayShareBeans') {
								...
								console.log(
									`${(item.infoType != 10 && item.drawId && (typeArr[item.drawId] || item.drawId) + ':') || (item.value && item.value + ':') || ''}${
										item.infoName
									}`
								)
							}
