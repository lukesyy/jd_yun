...
if ($.isNode()) {
	...
	cookiesArr = [$.getdata('CookieJD'), $.getdata('CookieJD2'), ...$.toObj($.getdata('CookiesJD') || '[]').map(item => item.cookie)].filter(
		item => !!item
	)
}
!(async () => {
...
async function takeRequest(type) {
	...
	switch (type) {
		...
		case 'superBrandDoTask':
			if ($.runInfo.itemId === null) {
				...
				url = `https://api.m.jd.com/api?functionId=superBrandDoTask&appid=ProductZ4Brand&client=wh5&t=${Date.now()}&body=%7B%22source%22:%22secondfloor%22,%22activityId%22:${
					...
					$.oneTask.assignmentType
				},%22itemId%22:%22${$.runInfo.itemId}%22,%22actionType%22:0%7D`
			}
		...
		case 'help':
			url = `https://api.m.jd.com/api?functionId=superBrandDoTask&appid=ProductZ4Brand&client=wh5&t=${Date.now()}&body=%7B%22source%22:%22secondfloor%22,%22activityId%22:${
				$.activityId
			},%22encryptProjectId%22:%22${$.encryptProjectId}%22,%22encryptAssignmentId%22:%22${
				$.encryptAssignmentId
			}%22,%22assignmentType%22:2,%22itemId%22:%22${$.code}%22,%22actionType%22:0%7D`
			break
		default:
			console.log(`错误${type}`)
	}
